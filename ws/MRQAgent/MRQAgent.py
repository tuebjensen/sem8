...
class Hyperparameters:
    ...
    # Exploration
    buffer_size_before_training: int = int(10e3)
    exploration_noise: float = (
        0.2 * 0.5
    )  # 0.5 since we are using discrete actions which range from 0 to 1 instead of -1 to 1

    # TD3
    target_policy_noise: float = 0.2 * 0.5
    noise_clip: float = 0.3 * 0.5
...
class MRQAgent:
    ...
    def train_encoder(
        ...
        for i in range(self.enc_horizon):
            pred_done, pred_next_state_embedding, pred_reward = (
                self.encoder.mdp_predict(pred_state_embedding, action[:, i])
            )
    ...
    def train_rl(
        ...
        with torch.no_grad():
            next_state_embedding = self.encoder.state_embedder(next_state)
